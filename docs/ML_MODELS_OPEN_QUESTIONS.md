...
| 16 | Any **egress restriction on model outputs**? | **[documented 2026-08-22] yes, a severe one.** Notebook export **strips all cell outputs** except images; the S3 feature writes to an **SRE-owned** bucket and excludes Meta Content Library anyway | Embeddings and classifications **cannot leave as data** — only as rendered images. Now in `utilities.md` § "Getting Results Out" | **done, by reading** |
